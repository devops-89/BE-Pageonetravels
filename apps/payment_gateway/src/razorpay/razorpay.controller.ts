...
export class RazorpayController {
    ...
    async initiateHotelBooking(@Body() body:CreateHotelBookingDto,@Req() req:Request, @Res() res:Response){
        try{
            ...
            const amount=body.NetAmount;
            const hotelPayload=body;

            // save order
            const orderResponse=await this.orderRepositoryService.insertBooking(reference_id,"HOTEL",hotelPayload,amount,body.BookingCode);
