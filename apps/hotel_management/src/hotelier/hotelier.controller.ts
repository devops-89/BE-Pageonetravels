...
export class HotelierController {
    constructor(private readonly hotelierService: HotelierService, private readonly responseHandlerService: ResponseHandlerService) {}
    @Post('/login')
    async hotelierLogin(@Req() req: Request, @Res() res: Response, @Body() body: LoginDto) {
        try {
            //const device_type = req.headers['devicetype'];
            const result = await this.hotelierService.loginWithEmail(body);
